...
FROM idilic-prod AS queue-prod
...
COPY ${ROOTRELDIR}infra/redis/30-redis.ini /etc/php/7.4/apache2/conf.d/30-redis.ini
COPY ${ROOTRELDIR}infra/redis/30-redis.ini /etc/php/7.4/cli/conf.d/30-redis.ini
RUN set -eux;   \
	apt update; \
